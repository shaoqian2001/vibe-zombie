...
##
## Each building type has a unique layout: walls, floor, ceiling, and furniture
## arranged to give each shop/room a distinct feel.
...
const CEILING_COLOR  := Color(0.90, 0.88, 0.85)
const DOOR_COLOR     := Color(0.35, 0.22, 0.12)

# Furniture palettes per building type
const SHELF_COLOR    := Color(0.55, 0.40, 0.25)  # wood shelves
const COUNTER_COLOR  := Color(0.60, 0.58, 0.55)  # grey counter
const DESK_COLOR     := Color(0.50, 0.38, 0.28)  # office desk
const CRATE_COLOR    := Color(0.62, 0.50, 0.30)  # warehouse crate
const BOOTH_COLOR    := Color(0.65, 0.18, 0.15)  # diner booth red
const FRIDGE_COLOR   := Color(0.80, 0.82, 0.85)  # white fridge
const TABLE_COLOR    := Color(0.48, 0.35, 0.22)  # diner table
...
# The exit door Area3D for detecting when player wants to leave
var exit_area: Area3D

func setup(type: BuildingType, w: float, d: float, h: float) -> void:
	building_type = type
	interior_width = maxf(w, 4.0)
	interior_depth = maxf(d, 4.0)
	interior_height = maxf(h, 3.5)
	_generate_interior()
...
func _create_ceiling() -> void:
	_add_box(
		Vector3(0.0, interior_height, 0.0),
		Vector3(interior_width, 0.05, interior_depth),
		CEILING_COLOR, false
	)
...
func _create_exit_door() -> void:
	...
	shp.size = Vector3(1.4, 2.4, 1.0)
	cs.shape = shp
	exit_area.add_child(cs)
	exit_area.position = Vector3(0.0, 1.2, d * 0.5 + 0.5)
	add_child(exit_area)
...
func _create_interior_light() -> void:
	...
	# A second fill light for larger rooms
...
func _furnish_convenience_store() -> void:
	...
	# Checkout counter near the front-right
...
func _furnish_warehouse() -> void:
	...
	# Tall shelf rack against left wall
...
func _furnish_diner() -> void:
	...
	# Kitchen window in back wall (darker inset)
